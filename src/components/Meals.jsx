...
function Meals() {
  ...
  return (
    <div>
      ...
      <MealAreaSelector/>
      <div className="h-[80vh] mt-2 w-full gap-1 items-start justify-center grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {meals.map((meal, index) => (
          <MealItem key={index} image={meal.strMealThumb} name={meal.strMeal} />
        ))}
      </div>
    </div>
  );
